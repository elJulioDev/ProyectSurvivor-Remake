...
func _activate() -> bool:
	...
	for i in range(num_projectiles):
		...
		_spawn_projectile(spawn_pos, angle, 9.0 * speed_mult,
			final_dmg, 9999 + extra_pen, 100, size_mult)
...
func _spawn_nova_flash() -> void:
	...
	if pn: pn.add_child(flash)
	else:  get_parent().add_child(flash)
...
func _spawn_projectile(pos: Vector2, angle: float, speed: float,
	...
	p.radius        = maxf(3.0, 9.0 * sz_mult)
	var pn: Node = get_tree().get_first_node_in_group("projectiles")
	if pn: pn.add_child(p)
	else:  get_parent().add_child(p)

class _NovaProjectile extends Node2D:
	...
	func _draw() -> void:
		...
		var a: float = clampf(progress * 1.1, 0.0, 1.0)
		draw_circle(Vector2.ZERO, radius, Color(0.86, 0.31, 1.0, a))
		draw_circle(Vector2.ZERO, maxf(2.0, radius * 0.45), Color(1.0, 0.8, 1.0, a))

class _NovaFlash extends Node2D:
	var timer: float = 0.0
	var max_t: float = 15.0
	var max_r: float = 90.0
	func _ready()    -> void: z_index = 0
	func _process(delta: float) -> void:
		timer += delta * 60.0; queue_redraw()
		if timer >= max_t: queue_free()
	func _draw() -> void:
