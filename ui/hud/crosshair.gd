...
func _process(delta: float) -> void:
	...
	if is_instance_valid(player) and player.weapons.size() > 0:
		...
		# CAMBIO 2: Ahora leemos el cooldown desde el WeaponController
		if player._weapon_controller and player._weapon_controller.cooldowns.has(current_weapon):
			var current_cooldown = player._weapon_controller.cooldowns[current_weapon]
			
			# Comparamos el cooldown actual con el del frame anterior
			if current_cooldown > _last_cooldown and current_cooldown > 0:
				var shake = current_weapon.shake_amount if "shake_amount" in current_weapon else 0.0
				crosshair_scale += 0.3 + (shake * 0.15)
				
			_last_cooldown = current_cooldown
